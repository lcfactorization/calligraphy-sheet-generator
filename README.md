...
[![Version](https://img.shields.io/badge/version-2.9.7-blue)](CHANGELOG.md)
...
- **最新版本**:v2.9.7(引导增强 9 步 + "不再自动弹出"选项 + 智能推荐说明 + Dark 模式范字 inverted color)
- **最新更新**:v2.9.7 — 引导从 5 步扩展到 9 步(新增字体选择/自定义字体/智能推荐/导入生词文件);引导浮层新增"不再自动弹出"复选框 + 设置中心开关;每步标注 🔄/✋ 是否自动刷新字帖;修复 Dark 模式下 SVG 范字硬编码黑色不可见问题(`currentColor` + dark 自动反色 + 打印强制黑色)。详见 [CHANGELOG.md](./CHANGELOG.md)
...
├── package.json             ← 依赖配置(v2.9.7,ES Module)
...
├── CHANGELOG.md             ← 更新日志(v1.0.1 → v2.9.7 全记录)
