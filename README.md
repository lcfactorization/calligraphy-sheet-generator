# 字帖生成器 — 独立HTML + Puppeteer PDF 矢量生成工具

> 双轨方案：浏览器直接打印（全平台含 Android） + Puppeteer 命令行批量生成（桌面端自动化）

## 目录结构

```
distribution/
├── 字帖生成器.html          ← 主文件（独立HTML，全平台浏览器直接打开即用）
├── puppeteer-pdf.js         ← Puppeteer PDF矢量生成脚本（桌面端Node.js工具）
├── package.json             ← Node.js依赖配置
├── README.md                ← 本文档
├── CHANGELOG.md             ← 更新日志
└── fonts/                   ← 字体文件夹（可添加自定义字体）
    ├── 姜浩硬笔楷书.ttf      ← 默认汉字字体
    ├── texgyreadventor-regular.otf ← 拼音字体（已同时base64内嵌到HTML）
    ├── STKAITI.TTF           ← 华文楷体
    ├── FZFSB.TTF             ← 方正仿宋GBK
    ├── FZSJ-DQYBKSJW.TTF     ← 方正宋简大漆
    ├── FZSJ-HAITWY.TTF       ← 方正宋简海豚
    ├── TW-Kai.ttf            ← 文鼎楷体
    ├── 我逸清晨体楷书.ttf
    └── 田英章楷书30Light.otf
```
...
# 指定字体和页面格式
node puppeteer-pdf.js -t "静夜思" --font 华文楷体 --format a3 --landscape
```
...
| 字体名称 | CSS字体族 |
|----------|-----------|
| 姜浩硬笔楷书（默认） | JiangHaoYingBiKaiShu |
| 华文楷体 | STKaiti |
| 方正仿宋GBK | FZFSGBK |
