...
- **双引擎 AI 适配**：AI 组词补齐功能支持 DeepSeek 和火山引擎豆包两种大模型，根据 API Key 前缀自动识别引擎类型
  ...
  - API Key 以 `ark-` 开头 → 自动使用火山引擎豆包（doubao-seed-2-0-lite-260428，免费）
  - 其它前缀无法识别，保存时给出警告，运行时抛错提示
  - 火山引擎端点：`https://ark.cn-beijing.volces.com/api/v3/chat/completions`
  - 无需手动切换设置，用户只需粘贴任一平台的 API Key 即可
  - 设置面板标签更新为"DeepSeek 或火山引擎 API Key"，placeholder 提示两种格式（sk-... / ark-...）
  - 保存 API Key 时自动标注识别到的引擎类型
  - 新增 `detectApiKeyType(key)` 和 `getAiProvider(key)` 导出函数
- **三项独立功能开关**（默认全关闭，按需勾选，避免全量检查默认打开导致效率低、测试评审效果差）
  - 全量检查（优先级 1）：核验所有未缓存字的拼音/语义/已有组词，不匹配则纠错
  - 组词补齐（优先级 2）：仅补齐默认词库缺失二字词的字（"组词"占位）
  - 拼音纠错（优先级 3）：仅核对多音字/误读并纠正拼音
  - 都不勾选时点"AI 检查与补齐"等同"组词补齐"
  - 作用域优先级：全量检查(全部) > 组词补齐(仅缺失) > 拼音纠错(全部仅拼音)
  - 已写入 AI 缓存的字会跳过，避免重复消耗
- **进度提示**：AI 处理过程中状态栏显示旋转图标（SMIL animateTransform）+ 批次进度（已处理/总数 · 批次 N/M）
  - `fillMissingZuci` 新增 `onProgress(info)` 回调，每批完成后触发
  - 旋转图标无需 CSS keyframes，纯 SVG 实现
- **简化引擎显示**：状态消息中引擎标签简化为 `DeepSeek` / `豆包`，避免 `DeepSeek V4 Flash` 等冗长重复
- **眼睛图标低调化**：API Key 显示/隐藏切换按钮使用暗淡 SVG 图标（隐藏态=带斜线眼-off，显示态=眼-on）
- **AI 调用解读引导步骤**：导航引导从 17 步扩展到 18 步，追加"AI 调用流程解读"步骤
  - 解读点击运行后的完整流程：引擎识别→分批处理→进度提示→结果解读→开关选择建议→缓存机制→概率属性
  - 锚点 `#scAiSection`（AI 区域 section，始终可见，不依赖"启用 AI 补齐"开关状态）
- **健壮 JSON 提取**：适配豆包 lite 等弱模型可能输出 markdown 代码块或多余文字的场景
  - `extractJsonRobust(content)` 依次尝试：直接解析 → 剥离 ```json 代码块 → 提取首个 {...} → 首个 [..] 数组

### 优化
- 火山引擎 Chat API 不支持 `response_format` 参数，仅 DeepSeek 添加该参数
- AI 提示词适配豆包 lite：更简短、更直白、强调"只输出 JSON"
- 缓存条目结构扩展：`{ zuci, pinyin, pinyinFixed, wordsDetail, ts }`

### 版本号
- 主版本号从 2.9.9 升级至 3.0.0（引入双引擎适配为重大功能增强）

### 新增功能（2026-08-07 AI 加固）
- AI 三模式分流：fast（仅补齐）/ single_check（单音字校验）/ poly_check（多音字深度校验，自动升级强模型）
- AI 调用健壮性：每批最多 3 次指数退避重试 + 5 分钟硬超时 + 超时部分成功保留 + 中文错误诊断建议
- AI 开关联动：启用默认勾"组词补齐"；勾"全量检查"自动联动其余两项并灰显 + 黄色耗时预警条
- 紧急逃生门：localStorage `ai_model_override` 可免打包切换模型

### 修复（2026-08-07 AI 加固）
- 缓存穿透 Bug：全量检查/拼音纠错对已有组词缓存的字不再误跳过（新增 pinyinChecked 缓存字段）
- 多音字判定兼容：pinyin-pro `multiple:true` 返回空格分隔字符串而非数组，双判修复
- 进度统计改用已处理字数（processedCount），AI 返回异常时进度不再停滞
- 仅拼音纠错后的字再次组词补齐时不再被误跳过，且不再覆盖已有拼音纠正记录
- AbortSignal.any 旧 WebView 兼容兜底（MatePad 等移动端）
