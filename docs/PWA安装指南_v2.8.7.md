# 字帖生成器 v2.8.7 PWA 安装指南

> 版本：v2.8.7
> 更新日期：2026-07-25
> 在线地址：https://lcfactorization.github.io/calligraphy-sheet-generator/
> 打印调试地址：https://lcfactorization.github.io/calligraphy-sheet-generator/?printdebug=1
...
> **打印调试测试链接**
>
> 如需排查移动端打印分页问题，请访问带 `?printdebug=1` 参数的测试链接：
>
> **https://lcfactorization.github.io/calligraphy-sheet-generator/?printdebug=1**
>
> 该链接会在页面右下角显示半透明日志浮层，实时输出 `section` 数量、`page-break` 数量、`window.print()` 调用前后时间戳、UA、视口尺寸等关键诊断信息（v2.8.7 新增）。
...
> [!WARNING]
> iOS Safari 不支持 `window.print()` 直接调起系统打印，需通过「分享」→「打印」走 AirPrint 路径。v2.8.7 已针对此路径优化打印样式表（.print-row-pair 行对防撕 + 每页首行页顶线）。
...
> **v2.8.7 打印分页真机验收清单**
>
> 除上述 PWA 基础验证外，v2.8.7 增加以下移动端打印分页真机验收项：
>
> - [ ] 198 字字帖严格输出 18 页（无空白末页）
> - [ ] 每页页眉（日期/标题/字体）与页脚（评分/页码）齐全
> - [ ] 拼音行与汉字行作为行对在同一页内，不被拆到两页
> - [ ] 二次打印分页正常（首次打印后再次打印仍为 18 页）
> - [ ] 访问 `?printdebug=1` 测试链接，右下角日志浮层显示正常
...
> [!NOTE]
> v2.8.7 已通过真实 DOM 节点 + 固定几何（header 8mm + content 261mm + footer 8mm = 277mm）修复此问题。

排查步骤：
1. 确认应用版本为 v2.8.7（页面底部可见版本号）
...
> [!WARNING]
> 微信 X5 内核默认拦截 `window.print()`，v2.8.7 通过 iframe 降级方案修复。
...
### 问题 6：移动端打印分页错乱 / 二次打印失效 / 拼音撕页

> [!IMPORTANT]
> v2.8.7 已针对这 3 类根因（RC1/RC4/RC5）集中修复。

排查步骤：
1. 确认应用版本为 v2.8.7（页面底部可见版本号）
2. 访问 `https://lcfactorization.github.io/calligraphy-sheet-generator/?printdebug=1`，观察右下角日志浮层
3. 确认日志中 `section` 数量为 18、`page-break` 数量为 17
4. 清空缓存后重新访问以加载最新 SW
5. 若仍异常，记录日志浮层内容并在 GitHub 仓库提交 Issue

---

> [!IMPORTANT]
> 本指南适用于 v2.8.7 版本。如遇其他问题，请在 GitHub 仓库提交 Issue：https://github.com/lcfactorization/calligraphy-sheet-generator/issues
